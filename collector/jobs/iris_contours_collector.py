...
class IrisContoursCollectorJob(BaseCollectorJob):
    """
    Collecteur pour les contours géographiques IRIS.
    """

    def __init__(
            self,
            contours_url: str = "https://data.geopf.fr/telechargement/download/CONTOURS-IRIS/CONTOURS-IRIS_3-0__GPKG_LAMB93_FXX_2024-01-01/CONTOURS-IRIS_3-0__GPKG_LAMB93_FXX_2024-01-01.7z",
            **kwargs
    ):
        """
        ...
        Args :
            contours_url : URL du fichier de contours IRIS
        """
        super().__init__(
        ...
        self.contours_url = contours_url
        self.temp_dir = None
    ...
    def _collect_data(self, **kwargs) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"Téléchargement des contours IRIS depuis {self.contours_url}")
        ...
        try:
            # Téléchargement
            response = requests.get(self.contours_url, stream=True)
            ...
            if not source_file:
                error_msg = "Aucun fichier géographique (.gpkg) trouvé"
                raise Exception(error_msg)

            self.logger.info(f"Fichier géographique trouvé: {source_file} (format: {source_format})")

            return {
                'source_file': source_file,
                'source_format': source_format,
                'source_url': self.contours_url,
                'all_files': all_files
            }
    ...
    def _save_data(self, processed_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        """
        self.logger.info("Sauvegarde des contours IRIS dans le format original")
        ...
        content_type = 'application/geopackage+sqlite3' if source_format == 'gpkg' else 'application/x-esri-shape'

        # Chemin du fichier incluant le dossier daté
        file_name = f"iris_contours.{source_format}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        collector = IrisContoursCollectorJob(
